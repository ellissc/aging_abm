...
Language change can be understood as representing the dynamic relationship between group-level and individual usage patterns. According to statistical learning, language learners are sensitive to the underlying distributional properties of language usage. Therefore, when usage patterns vary across time, it would be expected that the language acquisition of individuals will be impacted by the usage patterns that they experience during their lifetime. While previous research has shown differences in the language representation and cognitive capabilities over the lifespan (i.e., early vocab spurt, late cognitive decline), the impact of varying learning rates on language change is not well understood. Using an agent-based model of (syntactic) language change, this project aims to explore the interaction between learning rates and aging may impact diachronic changes in usage patterns of language.
...
**TO UPDATE: Growth curve, just decreasing now.**
...
- Communication
    ...
    - In these analyses, Alpha will be set to 0. No bias for grammar 1.
...
*Use this style of summary graph*

```{r gamma-variations, echo = F}
gvdf <- read_csv(here("paper",
                      "processed_data",
                      "gvdf_final-only.csv"),
                 show_col_types = F)
...
lndf2 <- read_csv(here("code",
                      "Language Change prob-gamma-final-only-table.csv"),
...
#     ggtitle("Probabilistic gamma / learning rates")

ggplot(lndf2, aes(x = percent_grammar_1, y = mean_state_of_nodes,
                 color = factor(initial_gamma)))+
    stat_summary(fun = mean, geom = "point")+
    stat_summary(fun = mean, geom = "line")+
    stat_summary(fun.data = mean_se, geom = "errorbar", width = 2)+
    ...
    theme_bw()+
    # facet_grid(percent_grammar_1~initial_gamma)+
    ggtitle("Probabilistic gamma / learning rates", subtitle = "Gamma SD = 0.02, Grammar 1 bias = 0.005 (weak)")
```
...
lndf3 <- read_csv(here("code",
                      "Language Change age-mod-gamma-final-only-table.csv"),
                 show_col_types = F, skip = 6) |> 
    janitor::clean_names()
...
#     ggtitle("Age (deterministic) modifies gamma / learning rate")

ggplot(lndf3, aes(x = percent_grammar_1, y = mean_state_of_nodes))+
    stat_summary(fun = mean, geom = "point")+
    stat_summary(fun = mean, geom = "line")+
    stat_summary(fun.data = mean_se, geom = "errorbar", width = 2)+
    ylab("Mean state of nodes\nafter 1000 steps")+
    ...
    scale_color_brewer(name = "Grammar 1\nstarting percentage", type = "qual" ,
                       palette = 3)+
    theme_bw()+
    ggtitle("Gamma determined by age (deterministic)", subtitle  = "Linear positive relationship")
```
...
lndf4 <- read_csv(here("code",
                      "Language Change age-vary-mod-gamma-final-only-table.csv"),
                 show_col_types = F, skip = 6) |> 
    janitor::clean_names()
...
#     ggtitle("Age (probabilistic) modifies gamma / learning rate")

ggplot(lndf4, aes(x = percent_grammar_1, y = mean_state_of_nodes, color = factor(age_sd)))+
    stat_summary(fun = mean, geom = "point")+
    stat_summary(fun = mean, geom = "line")+
    stat_summary(fun.data = mean_se, geom = "errorbar", width = 2)+
    ylab("Mean state of nodes\nafter 1000 steps")+
    xlab("Initial percentage of Grammar 1")+
    scale_color_brewer(name = "Age SD",# type = "qual" ,
                       palette = 5)+
    theme_bw()+
    # facet_wrap(~factor(age_sd))+
    ggtitle("Gamma determined by age (probabilistic)", subtitle  = "Linear positive relationship")


```

```{r vary-growth-curve, echo = F}
lndf5 <- read_csv(here("code",
                      "Language Change vary-growth-curve-final-only-table.csv"),
                 show_col_types = F, skip = 6) |> 
    janitor::clean_names()

ggplot(lndf5, aes(x = percent_grammar_1, y = mean_state_of_nodes, color = gamma_with_age))+
    stat_summary(fun = mean, geom = "point")+
    stat_summary(fun = mean, geom = "line")+
    stat_summary(fun.data = mean_se, geom = "errorbar", width = 2)+
    ylab("Mean state of nodes\nafter 1000 steps")+
    xlab("Initial percentage of Grammar 1")+
    scale_color_brewer(name = "Relationship", type = "qual" ,
                       palette = 3)+
    theme_bw()+
    # facet_grid(~gamma_with_age)+
    ggtitle("Growth curve type", subtitle = "Gamma increases/decreases with age")
...
```{r vary-growth-curve-cost, echo = F}
lndf6 <- read_csv(here("code",
                      "Language Change vary-growth-curve-cost-final-only-table.csv"),
                 show_col_types = F, skip = 6) |> 
    janitor::clean_names()

ggplot(lndf6, aes(x = percent_grammar_1, y = mean_state_of_nodes,
                  color = factor(gamma_cost)))+
    stat_summary(fun = mean, geom = "point")+
    stat_summary(fun = mean, geom = "line")+
    stat_summary(fun.data = mean_se, geom = "errorbar", width = 2)+
    ylab("Mean state of nodes\nafter 1000 steps")+
    xlab("Initial percentage of Grammar 1")+
    scale_color_brewer(name = "Gamma cost",# type = "qual" ,
                       palette = 5)+
    theme_bw()+
    facet_grid(gamma_with_age~.)+
    ggtitle("Varied growth curve cost")
...
<!-- ```{r vary-growth-curve-decreases, echo = F} -->
<!-- lndf6.5 <- read_csv(here("code", -->
<!--                       "Language Change vary-growth-curve-decreases-table.csv"), -->
<!--                  show_col_types = F, skip = 6) |>  -->
<!--     janitor::clean_names() -->

<!-- ggplot(lndf6.5, aes(x = step, y = mean_state_of_nodes, group = run_number, -->
<!--                   color = factor(percent_grammar_1)))+ -->
<!--     geom_line(alpha = 0.5)+ -->
<!--     scale_color_brewer(name = "Grammar 1\nstarting percentage", type = "qual" , -->
<!--                        palette = 3)+ -->
<!--     theme_bw()+ -->
<!--     facet_grid(gamma_cost~percent_grammar_1)+ -->
<!--     ggtitle("Varied growth curve cost", subtitle = "Decreasing gamma only") -->
...
lndf7 <- read_csv(here("code",
                      "Language Change half-split-age-final-only-table.csv"),
                 show_col_types = F, skip = 6) |> 
    janitor::clean_names()

ggplot(lndf7, aes(x = percent_grammar_1, y = mean_state_of_nodes,
                  color = factor(group2_age)))+
    stat_summary(fun = mean, geom = "point")+
    stat_summary(fun = mean, geom = "line")+
    stat_summary(fun.data = mean_se, geom = "errorbar", width = 2)+
    ylab("Mean state of nodes\nafter 1000 steps")+
    xlab("Initial percentage of Grammar 1")+
    scale_color_brewer(name = "Group 2 age", type = "qual" ,
                       palette = 3)+
    theme_bw()+
    facet_grid(percent_group2~.)+
    ggtitle("Two age groups")

```

Group preference:

```{r cohort-preference, echo = F}

lndf8 <- read_csv(here("code",
                      "Language Change cohort-preference-final-only-table.csv"),
                 show_col_types = F, skip = 6) |> 
    janitor::clean_names()

ggplot(lndf8, aes(x = percent_grammar_1, y = mean_state_of_nodes,
    ...
    ylab("Mean state of nodes\nafter 1000 steps")+
    xlab("Initial percentage of Grammar 1")+
    scale_color_brewer(name = "Chance to listen\nto outgroup", type = "div",
                       palette = 3)+
    theme_bw()+
    facet_wrap(~percent_group2)+
    ggtitle("Willingness to listen to outgroup", subtitle = "Faceted by group 2 percent")

```



**Other things to consider: Time to equilibrium, emergence of dialect subgroups**
**Growth curve cost?**
